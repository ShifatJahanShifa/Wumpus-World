  ...
  */
  ...
  private getPossibleDirections(): number[] {
    ...
    private updateBoardAfterWumpusShot(row: number, column: number): void {
      ...
      // Remove stench from the affected cell
      this.removeStench(row, column);
    }
  
  // Assuming getAvailableDirections is implemented as follows:
  ...
  move() {
    ...
    // Determine the affected cell based on shootDirection
    ...
    // cell is danger cell
      ...
      if (visitedAvailableDirections.length > 0) {
          return this.chooseRandomDirection(visitedAvailableDirections);
      }
      ...
      if (unvisitedAvailableDirections.length > 0) {
          return this.chooseRandomDirection(unvisitedAvailableDirections);
      }
  
      // Move randomly
      const availableDirections = this.getAvailableDirections(() => true);
      if (availableDirections.length > 0) {
          return this.chooseRandomDirection(availableDirections);
      }
