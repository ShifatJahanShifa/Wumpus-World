class Play{
    ...
    */
    ...
    private getPossibleDirections(): number[] {
      ...
      private getDirectionsBelowThreshold():  { direction: number; probability: number }[] {
        const belowThreshold: { direction: number; probability: number }[] = [];
        if (
          this.agentIndex.row < 9 && (this.totalMoves[this.agentIndex.row+1][this.agentIndex.column]<15 ||
        ...
        if (
          this.agentIndex.column < 9 && (this.totalMoves[this.agentIndex.row][this.agentIndex.column+1]<15 ||
        ...
        if (
          this.agentIndex.row > 0 && (this.totalMoves[this.agentIndex.row-1][this.agentIndex.column]<15 ||
            !this.cellVisited[this.agentIndex.row-1][this.agentIndex.column]) &&
          this.pitProbability[this.agentIndex.row - 1][this.agentIndex.column] < this.threshold
        ) {
        ...
        if (
          this.agentIndex.column > 0 && (this.totalMoves[this.agentIndex.row][this.agentIndex.column-1]<15 ||
            !this.cellVisited[this.agentIndex.row][this.agentIndex.column-1]) &&
          this.pitProbability[this.agentIndex.row][this.agentIndex.column - 1] < this.threshold
        ) {
      ...
      private handlePitLoop(): number {
        ...
        const directionsBelowThreshold = this.getDirectionsBelowThreshold();
        if (directionsBelowThreshold.length > 0) {
          // Step 2: Sort by probability and take the minimum one
          //directionsBelowThreshold.sort((a, b) => a.probability - b.probability);
          //const bestDirection = directionsBelowThreshold[0].direction;
          const directions = directionsBelowThreshold.map(item => item.direction);
